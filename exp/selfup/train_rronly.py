...
@hydra.main(version_base=None, config_path="../conf", config_name="selfup_rronly")
def main(cfg):
    L.seed_everything(cfg.seed)
    torch.set_float32_matmul_precision("highest")

    model = SelfSupervisedRQTraining(cfg)
    ...
    prev_ckpt = cfg.get("prev_ckpt", None)
    trainer.fit(model=model, datamodule=data_module, ckpt_path=prev_ckpt)
