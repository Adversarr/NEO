...
def compute_surface_lbo(X, k=20, simple_mass=False):
    ...
    Returns:
        ...
        evals, evecs: First few eigenvalues/vectors.
    """
    N = X.shape[0]
    ...
    # Estimate local scale epsilon_i
    # Using the average distance to k-th neighbor as a robust bandwidth
    sigma = np.mean(dists, axis=1) 
    # Or median, widely used in heuristics:
    epsilon = np.median(sigma)**2 
    
    # 2. Build Gaussian Kernel W (Sparse)
    row = np.repeat(np.arange(N), k)
    col = indices.reshape(-1)
    sq_dists = dists.reshape(-1)**2
    
    # Standard Gaussian weights
    val = np.exp(-sq_dists / epsilon)
    W = sp.csr_matrix((val, (row, col)), shape=(N, N))
    
    # Symmetrize W (since kNN is not symmetric)
    W = (W + W.T) / 2.0
    
    # ========================================================
    # 3. The Crucial Step: Density Normalization (Coifman & Lafon)
    # ========================================================
    
    # Estimate density q(x)
    q = np.array(W.sum(axis=1)).flatten()
    
    # Inverse density matrix Q_inv
    Q_inv = sp.diags(1.0 / q)
    
    # Anisotropic kernel K: removing density bias
    # K_{ij} = W_{ij} / (q_i * q_j)
    K = Q_inv @ W @ Q_inv
    
    # ========================================================
    # 4. Construct Discrete Laplacian & Mass
    # ========================================================
    
    # Degree of the normalized kernel
    d_hat = np.array(K.sum(axis=1)).flatten()
    D_hat = sp.diags(d_hat)
    
    # In FEM context L_fem = D - W.
    # Here, our normalized K acts as the geometry-aware adjacency.
    # "Unnormalized" Laplacian L = D_hat - K is positive semi-definite.
    L = D_hat - K
    
    if simple_mass:
        M = sp.eye(N)
    else:
        # Proper Mass Matrix for Surface Integration
        # The measure on the manifold is approximated by d_hat.
        # Why? Because d_hat converges to the Riemannian volume measure.
        # So M should actually be D_hat for the generalized problem.
        M = D_hat
