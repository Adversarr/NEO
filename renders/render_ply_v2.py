...
def setup_world_fog(color=(1.0, 1.0, 1.0), density=0.0, anisotropy=0.0):
    scene = bpy.context.scene
    if scene.world is None:
        scene.world = bpy.data.worlds.new(name="World")
    world = scene.world
    ...
    if output_node is None:
        output_node = nodes.new(type="ShaderNodeOutputWorld")
    try:
        volume_in = output_node.inputs["Volume"]
    except Exception:
        volume_in = None
    fog_node = nodes.new(type="ShaderNodeVolumeScatter")
    if "Color" in fog_node.inputs:
        fog_node.inputs["Color"].default_value = (*tuple(color), 1.0)
    if "Density" in fog_node.inputs:
        fog_node.inputs["Density"].default_value = float(density)
    if "Anisotropy" in fog_node.inputs:
        fog_node.inputs["Anisotropy"].default_value = float(anisotropy)
    if volume_in is not None:
        has_volume_link = any(link.to_node == output_node and link.to_socket == volume_in for link in links)
        if not has_volume_link:
            try:
                links.new(fog_node.outputs["Volume"], volume_in)
            except Exception:
                pass
...
def create_shader_material(
    ...
    attr_node.attribute_type = 'INSTANCER' # Key: Instancer type
    attr_node.attribute_name = "output_color" # Must match the name in the geometry nodes
...
def import_and_process_ply(
    ...
    # Create geometry nodes and apply material
...
def main():
    ...
    parser.add_argument('--auto_light_energy_scale', type=float, default=1.0, help="Scale factor for auto light energy")
    parser.add_argument('--auto_light_energy_ref_radius', type=float, default=1.0, help="Reference radius for auto light energy scaling")
    parser.add_argument('--auto_light_energy_max_scale', type=float, default=100.0, help="Maximum scale for auto light energy")
    ...
    parser.add_argument('--emission_strength', type=float, default=0.0, help="Principled emission strength")
    ...
    parser.add_argument('--fog_density', type=float, default=0.0, help="World volumetric fog density")
    parser.add_argument('--fog_color', nargs=3, type=float, default=[1.0, 1.0, 1.0], help="World volumetric fog RGB")
    parser.add_argument('--fog_anisotropy', type=float, default=0.0, help="World volumetric fog anisotropy")
    parser.add_argument('--balanced_lights', action='store_true', help="Add balanced fill and rim lights")
    parser.add_argument('--fill_light_energy', type=float, default=600.0, help="Fill area light energy")
    parser.add_argument('--fill_light_color', nargs=3, type=float, default=[1.0, 1.0, 1.0], help="Fill area light RGB")
    parser.add_argument('--rim_light_energy', type=float, default=400.0, help="Rim area light energy")
    parser.add_argument('--rim_light_color', nargs=3, type=float, default=[1.0, 1.0, 1.0], help="Rim area light RGB")
    ...
    if args.swap_yz:
        print("Swapping Y and Z coordinates...")
        # Swap Y and Z by rotating -90 degrees around X axis
        # and then applying the transformation to vertices
        import mathutils
    ...
    if args.obj_rot_deg is not None:
        print(f"Applying custom rotation: {args.obj_rot_deg} degrees...")
        import mathutils
    ...
    obj_center, obj_radius = _get_object_center_and_radius(obj)
    obj_min, _ = _get_object_bounds(obj)
    ...
    if args.fog_density > 0.0:
        setup_world_fog(color=tuple(args.fog_color), density=args.fog_density, anisotropy=args.fog_anisotropy)
    ...
    if not args.no_lights:
        ...
        point_light_energy = float(args.point_light_energy)
        area_light_energy = float(args.area_light_energy)
        fill_light_energy = float(args.fill_light_energy)
        rim_light_energy = float(args.rim_light_energy)
        ...
        if args.auto_frame and args.auto_lights:
            ...
            ref_r = max(float(args.auto_light_energy_ref_radius), 1e-6)
            max_scale = max(float(args.auto_light_energy_max_scale), 1.0)
            energy_scale = float(args.auto_light_energy_scale) * (r / ref_r) * (r / ref_r)
            energy_scale = min(energy_scale, max_scale)
            point_light_energy *= energy_scale
            area_light_energy *= energy_scale
            fill_light_energy *= energy_scale
            rim_light_energy *= energy_scale
        ...
        setup_light(
            point_light_pos,
            energy=point_light_energy,
            type="POINT",
            color=tuple(args.point_light_color),
            shadow_soft_size=point_light_soft_size,
        )

        if args.balanced_lights:
            from mathutils import Vector
            r = max(float(obj_radius), 1e-6)
            c = Vector(obj_center)
            v_cam = Vector(cam_pos) - c
            if v_cam.length == 0:
                v_cam = Vector((0.0, -1.0, 0.25))
            v_cam.normalize()
            ...
            side.normalize()
            fill_pos = c - v_cam * (1.6 * r) + side * (1.2 * r) + up * (0.8 * r)
            rim_pos = c + v_cam * (2.4 * r) + up * (1.5 * r)
            fill_rot = _look_at_euler((float(fill_pos.x), float(fill_pos.y), float(fill_pos.z)), obj_center)
            rim_rot = _look_at_euler((float(rim_pos.x), float(rim_pos.y), float(rim_pos.z)), obj_center)
            setup_light(
                (float(fill_pos.x), float(fill_pos.y), float(fill_pos.z)),
                energy=fill_light_energy,
                type="AREA",
                rotation_euler=fill_rot,
                color=tuple(args.fill_light_color),
                size=area_light_size,
                size_y=area_light_size_y,
                spread=args.area_light_spread,
            )
            setup_light(
                (float(rim_pos.x), float(rim_pos.y), float(rim_pos.z)),
                energy=rim_light_energy,
                type="AREA",
                rotation_euler=rim_rot,
                color=tuple(args.rim_light_color),
                size=area_light_size,
                size_y=area_light_size_y,
                spread=args.area_light_spread,
            )
