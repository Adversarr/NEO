"""Multi-resolution point cloud sampling for multiple meshes."""
...
from g2pt.utils.mesh_feats import mesh_laplacian, point_cloud_laplacian, sample_points_uniformly, solve_gev


def parse_args():
    parser = argparse.ArgumentParser(description="Sample points at multiple resolutions for multiple meshes.")
    ...
    parser.add_argument(
        "--n_points", type=int, nargs="+", default=[2048, 8192, 32768], help="List of resolutions to sample."
    )
    parser.add_argument(
        "--k", type=int, default=128, help="Number of eigenfunctions to compute (should match model's expected k)."
    )
    parser.add_argument("--seed", type=int, default=None, help="Random seed for reproducibility.")
    parser.add_argument("--normalize", action="store_true", help="Normalize points to unit sphere.")
    ...
    for mesh_file in mesh_files:
        ...
        for n in args.n_points:
            ...
            points, face_index = sample_points_uniformly(
                vertices, faces, number_of_points=int(n), seed=args.seed, return_face_index=True
            )
