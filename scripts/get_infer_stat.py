...
def collect_statistics(data_dir: Path) -> dict:
    ...
    for sample_dir in sample_dirs:
        results_file = sample_dir / "inferred" / "results.json"
        has_results_file_fsp = (sample_dir / "inferred" / "fastspectrum_results.json").exists()
        if not results_file.exists() or not has_results_file_fsp:
            print(f"Warning: {results_file} or {sample_dir / 'inferred' / 'fastspectrum_results.json'} does not exist, skipping.")
            continue
        ...
        if precisions:
            ...
            for p_name, p_data in precisions.items():
                ...
                if pointcloud_vs_network is None:
                    pointcloud_vs_network = scores.get("mesh_vs_network")
                ...
                if "network_gev" in times: all_precision_stats[p_name]["network_gev_times"].append(times["network_gev"])
