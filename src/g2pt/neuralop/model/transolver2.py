...
from g2pt.neuralop.layers.attn.transolver2 import (
    Transolver2CrossAttention_ShareKV,
    Transolver2SelfAttention,
)
...
class Transolver2Block(nn.Module):
    def __init__(
        self,
        # attention
        d_model: int,
        ...
        super().__init__()
        self.attn = Transolver2SelfAttention(d_model, phys_dim, attn)
    ...
    def forward(
        ...
        fx_a, reg_tok_fx_a = self.attn(fx, x, reg_tok_fx, reg_tok_x, mass)  # B, nPoint, C
        fx = fx_a + fx
        reg_tok_fx = reg_tok_fx_a + reg_tok_fx

        fx = fx + self.ffn(self.ffn_ln(fx))  # B, nPoint, C
        reg_tok_fx = reg_tok_fx + self.ffn(self.ffn_ln(reg_tok_fx))  # B, nRegisterTokens, C
        return fx, reg_tok_fx

class Transolver2CrossBlock(nn.Module):
    def __init__(
        self,
        phys_dim: int,
        ...
        super().__init__()
        self.attn = Transolver2SelfAttention(d_model=q_dim, phys_dim=phys_dim, config=attn)

        self.cross = Transolver2CrossAttention_ShareKV(kv_dim, q_dim, phys_dim, q_dim, attn)
    ...
    def forward(
        ...
        y = qx + y_a
        reg_tok_qx = reg_tok_qx + reg_tok_qx_a

        y_c, reg_tok_qx_c = self.cross(y, kvx, x, reg_tok_qx, reg_tok_x, x_kv, mass_q, mass_kv)
        y = y + y_c
        reg_tok_qx = reg_tok_qx + reg_tok_qx_c

        y = y + self.ffn(self.ffn_ln(y))  # B, nPoint, C
        reg_tok_qx = reg_tok_qx + self.ffn(self.ffn_ln(reg_tok_qx))  # B, nRegisterTokens, C
        return y, reg_tok_qx

@dataclass
class Transolver2ModelConfig:
    d_model: int
    num_layers: int
    num_registers: int
    # lifting part
...
class Transolver2Model(nn.Module):
    def __init__(
        self,
        phys_dim: int,
        func_dim: int,
        out_dim: int,
        config: Transolver2ModelConfig,
    ) -> None:
        super().__init__()
    ...
    def reset_parameters(self):
        """Reset the parameters of the Transolver2Model."""
    ...
    def forward(
        ...
        """
        Apply the Transolver2Model to the input tensor.
        ...
        regtok_x = self.register_tokens_x.expand(B, -1, -1).tanh() # [-1, 1]
        
        if mass is not None:
            if mass.dim() != 3 or mass.shape[-1] != 1 or mass.shape[0] != B or mass.shape[1] != x.shape[1]:
                raise ValueError(f"mass must be of shape (B, N, 1), got {mass.shape}")
        ...
        for block in self.blocks:
            x_hidden, regtok_fx = block(x, x_hidden, regtok_fx, regtok_x, m)  # B, nPoint, d_model
...
class Transolver2SolverModel(nn.Module):
    def __init__(
        self,
        phys_dim: int,
        q_dim: int,  # p-basis
    ...
    def reset_parameters(self):
        """Reset the parameters of the Transolver2SolverModel."""
        self.lifting.reset_parameters()
        self.project.reset_parameters()
        for block in self.blocks:
            block.reset_parameters()  # type: ignore
    ...
    def forward(
        ...
        """Apply Transolver2 solver to map p-basis to q-basis and solve RHS."""
        if mass_q is None:
