...
def axis_angle_to_matrix(axis_angle: torch.Tensor, fast: bool = False) -> torch.Tensor:
    ...
    # Ensure dtype-safe where: use ones_like to match tensor dtype
...
def random_rotate_3d(max_radius: float = 2.0 * np.pi) -> np.ndarray:
    """
    Generate a random 3D rotation matrix.

    Args:
        max_radius (float): Maximum angle in radians for the rotation. Default is 2π.

    Returns:
        np.ndarray: A 3x3 rotation matrix.
    """
    theta = (-0.5 + np.random.rand()) * max_radius  # Random angle in radians
    phi = (-0.5 + np.random.rand()) * max_radius  # Random angle in radians
    cos_theta, sin_theta = np.cos(theta), np.sin(theta)
    cos_phi, sin_phi = np.cos(phi), np.sin(phi)

    # Rotation matrix around the z-axis followed by y-axis
